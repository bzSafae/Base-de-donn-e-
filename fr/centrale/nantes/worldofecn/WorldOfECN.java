...
// Dans WorldOfECN.java
    ...
    public static void main(String[] args) {
        ...
        System.out.println("\n--- Test de Suppression ---");
        try {
            database.removeWorld(playerId, "mapartie", "Start");
            System.out.println("SUCCÈS: Sauvegarde 'Start' supprimée avec succès.");
        } catch (Exception e) {
            System.out.println("ERREUR lors de la suppression: " + e.getMessage());
            e.printStackTrace();
        }

        // --- TEST 4 (optionnel) : Vérifier que la sauvegarde a bien disparu ---
        System.out.println("\n--- Vérification post-suppression ---");
        World testAfterDelete = database.readWorld(playerId, "mapartie", "Start");
        if (testAfterDelete == null) {
            System.out.println("CONFIRMÉ ✅ : la sauvegarde 'Start' n'existe plus dans la base.");
        } else {
            System.out.println("⚠️ ERREUR : la sauvegarde semble encore présente !");
        }
    }
