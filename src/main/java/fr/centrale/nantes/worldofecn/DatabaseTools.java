...
public World readWorld(Integer idJoueur, String nomPartie, String nomSauvegarde) {
    ...
    public void removeWorld(Integer idJoueur, String nomPartie, String nomSauvegarde) {
        ...
        if (idJoueur == null || nomPartie == null || nomSauvegarde == null) {
        throw new IllegalArgumentException("Paramètres invalides : aucun ne doit être null.");
    }
    ...
    try {
        ...
        Integer idPartie = getPartie(idJoueur, nomPartie);
        if (idPartie == null) {
            System.out.println("removeWorld: Partie '" + nomPartie + "' non trouvée pour le joueur " + idJoueur);
            return;
        }

        // 2. Identifier la sauvegarde
        String querySave = "SELECT id_sauvegarde FROM sauvegarde WHERE id_partie = ? AND nom_sauvegarde = ?";
        Integer idSauvegarde = null;
        try (PreparedStatement stmt = connection.prepareStatement(querySave)) {
            stmt.setInt(1, idPartie);
            stmt.setString(2, nomSauvegarde);
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    System.out.println("removeWorld: Sauvegarde '" + nomSauvegarde + "' non trouvée.");
                    return;
                }
            }
        }
        ...
        // 3. Supprimer les associations
        String[] deleteLinks = {
            "DELETE FROM sauvegarde_personnage WHERE id_sauvegarde = ?",
            "DELETE FROM sauvegarde_monstre WHERE id_sauvegarde = ?",
            "DELETE FROM sauvegarde_objet WHERE id_sauvegarde = ?",
            "DELETE FROM sauvegarde_element WHERE id_sauvegarde = ?"
        };

        for (String sql : deleteLinks) {
        ...
        // 4. Supprimer la sauvegarde principale
        try (PreparedStatement stmt = connection.prepareStatement("DELETE FROM sauvegarde WHERE id_sauvegarde = ?")) {
            stmt.setInt(1, idSauvegarde);
            stmt.executeUpdate();
        }

        connection.commit();
        System.out.println("Sauvegarde '" + nomSauvegarde + "' supprimée avec succès.");

    } catch (SQLException ex) {
        Logger.getLogger(DatabaseTools.class.getName()).log(Level.SEVERE, "Erreur lors de removeWorld", ex);
        try {
            connection.rollback();
        } catch (SQLException rbe) {
            Logger.getLogger(DatabaseTools.class.getName()).log(Level.SEVERE, "Rollback échoué", rbe);
        }
    } finally {
        try {
            connection.setAutoCommit(true);
        } catch (SQLException e) {
            Logger.getLogger(DatabaseTools.class.getName()).log(Level.WARNING, "Impossible de remettre autoCommit=true", e);
        }
    }
    }
}
